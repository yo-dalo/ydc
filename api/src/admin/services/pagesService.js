...
class PagesService {
  ...
  static async getById(id, branchId) {
    return await pages.findOne({ where: { Id: id, Branch_Id: branchId } });
  }

  static async create(data, branchId) {
    const created = await pages.create({ ...data, Branch_Id: branchId });
    return created ? created.Id || created.id || created.get("Id") : null;
  }

  static async update(id, data, branchId) {
    const [affected] = await pages.update(data, { where: { Id: id, Branch_Id: branchId } });
    return affected > 0;
  }
  ...
  static async delete(id, branchId) {
    const deleted = await pages.destroy({ where: { Id: id, Branch_Id: branchId } });
    return deleted > 0;
  }
}
