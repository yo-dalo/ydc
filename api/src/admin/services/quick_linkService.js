...
class QuickLinkService {
  ...
  static async getById(id) {
    return await quick_link.findByPk(id);
  }

  static async create(data) {
    const created = await quick_link.create(data);
    return created ? created.Id || created.id || created.get("Id") : null;
  }

  static async update(id, data) {
    const [affected] = await quick_link.update(data, { where: { Id: id } });
    return affected > 0;
  }
  ...
  static async delete(id) {
    const deleted = await quick_link.destroy({ where: { Id: id } });
    return deleted > 0;
  }
}
