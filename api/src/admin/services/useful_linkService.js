...
class UsefulLinkService {
  ...
  static async getById(id, branchId) {
    return await useful_link.findOne({ where: { Id: id, Branch_Id: branchId } });
  }

  static async create(data, branchId) {
    const created = await useful_link.create({ ...data, Branch_Id: branchId });
    return created ? created.Id || created.id || created.get("Id") : null;
  }

  static async update(id, data, branchId) {
    const [affected] = await useful_link.update(data, { where: { Id: id, Branch_Id: branchId } });
    return affected > 0;
  }
  ...
  static async delete(id, branchId) {
    const deleted = await useful_link.destroy({ where: { Id: id, Branch_Id: branchId } });
    return deleted > 0;
  }
}

module.exports = UsefulLinkService;
