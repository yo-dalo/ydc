...
class PosterService {
  ...
  static async getById(id, branchId) {
    return await poster.findOne({ where: { Id: id, Branch_Id: branchId } });
  }

  static async create(data, Image, branchId) {
    const created = await poster.create({ ...data, Image, Branch_Id: branchId });
  ...
  static async update(id, data, Image, branchId) {
    ...
    const [affected] = await poster.update(updateData, { where: { Id: id, Branch_Id: branchId } });
    return affected > 0;
  }
