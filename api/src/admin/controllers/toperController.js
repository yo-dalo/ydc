...
exports.getById = async (req, res) => {
  try {
    const item = await ToperService.getById(req.params.id , req.admin.Branch_Id);
...
exports.getForUpdate = async (req, res) => {
  try {
    const item = await ToperService.getForUpdate(req.params.id , req.admin.Branch_Id  );
    if (!item) return errorResponse(res, "Toper not found", 404);
    return successResponse(res, "Toper fetched successfully", item);
  } catch (error) {
    return errorResponse(res, error.message);
...
exports.create = async (req, res) => {
  try {
    const id = await ToperService.create(req.body, req.file?.filename, req.admin.Branch_Id);
...
exports.update = async (req, res) => {
  try {
    const updated = await ToperService.update(req.params.id, req.body, req.file?.filename, req.admin.Branch_Id);
...
exports.delete = async (req, res) => {
  try {
    const deleted = await ToperService.delete(req.params.id, req.admin.Branch_Id);
