...
exports.getById = async (req, res) => {
  try {
    const item = await QuickLinkService.getById(req.params.id, req.admin.Branch_Id);
...
exports.getForUpdate = async (req, res) => {
  try {
    const item = await QuickLinkService.getForUpdate(req.params.id, req.admin.Branch_Id);
    if (!item) return errorResponse(res, "Quick Link not found", 404);
    return successResponse(res, "Quick Link fetched successfully", item);
  } catch (error) {
    return errorResponse(res, error.message);
...
exports.create = async (req, res) => {
  try {
    const id = await QuickLinkService.create(req.body, req.admin.Branch_Id);
...
exports.update = async (req, res) => {
  try {
    const updated = await QuickLinkService.update(req.params.id, req.body, req.admin.Branch_Id);
...
exports.delete = async (req, res) => {
  try {
    const deleted = await QuickLinkService.delete(req.params.id, req.admin.Branch_Id);
