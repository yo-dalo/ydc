...
exports.getById = async (req, res) => {
  try {
    const item = await BlogService.getById(req.params.id, req.admin.Branch_Id);
...
exports.getForUpdate = async (req, res) => {
  try {
    const item = await BlogService.getForUpdate(req.params.id, req.admin.Branch_Id);
    if (!item) return errorResponse(res, "Blog not found", 404);
    return successResponse(res, "Blog fetched successfully", item);
  } catch (error) {
    return errorResponse(res, error.message);
...
exports.create = async (req, res) => {
  ...
  try {
    const id = await BlogService.create(req.body, req.file?.filename, req.admin.Branch_Id);
...
exports.update = async (req, res) => {
  try {
    const updated = await BlogService.update(req.params.id, req.body, req.file?.filename, req.admin.Branch_Id);
...
exports.delete = async (req, res) => {
  try {
    const deleted = await BlogService.delete(req.params.id, req.admin.Branch_Id);
